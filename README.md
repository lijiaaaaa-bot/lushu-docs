...
- 空态：居中一句说明 + 可点胶囊 chips（含「海天乙方电费+停车费报告」）
- 消息只要短气泡：用户一句话摘要，助手短确认（已挂材料 N 份、缺 P/N、可生成）。**不**把任务卡全文或报告 markdown 贴进对话
- 材料：紧凑列表（短文件名 + 真表/文本），点进工作区看表结构
- 生成后：本地组装写入 `drafts/`（`.docx` + `.md`），对话只留「已生成专项报告 · 点此下载」；系统保存面板优先 .docx
- 底栏粘住：圆角胶囊输入，右侧实心胡桃「发送」
- 主 CTA（发送 / 生成文书 / 下载）胡桃实心胶囊；工作区为次级界面，默认不抢首页

色板不是豆包蓝，禁止系统蓝 CTA。挂上材料后可「生成文书」下载，工作区为次级界面。
