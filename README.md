...
- **材料卡片在用户气泡上方**（xlsx / pdf / docx：图标 + 文件名 + 类型），不是助手气泡下的 chip 条。
- 助手确认是纸面短文，不套紧白气泡。落稿后在奶油纸色上展开文书正文 + **近全宽下载卡**（标题 + `DOCX · 专项报告` + 右箭头，整卡可点，`home.download`）。聊天里不贴真表。
- 生成后给 2–3 条「相关问题」编号链，点选即发出短追问。不编造台账数字。
- `home.generate` 挂在最后一条可生成确认上（文字链）。下载不用细字链。
...
3. Run。打开即见左话题 + 右聊天。点「示例」后：用户气泡上方是文件卡片，助手是纸面短确认 +「生成文书」文字链。生成后纸面展开测算报告（年度金额来自真表）+ 近全宽下载卡（`home.download`）和「相关问题」。下载的 .docx 应是函件体，不是 xlsx 列名清单。
