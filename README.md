...
第一屏是 `HomeChatView`：居中对话栏 + 宽松空态。衬线「律书」仍是案匣品牌标题。

- 空态：居中说明 + 可点胶囊 chips（含「海天乙方电费+停车费报告」），可换行
- 消息：用户靠右、回执靠左；`card`/`paper` 底，brass / walnut 描边，**不是**豆包蓝气泡
- 底栏粘住：圆角胶囊输入，右侧实心胡桃「发送」
- 次级动作在输入上方：选材料 / 导入 / 示例 / 设置（图标+字小胶囊）
- 主 CTA（发送 / 生成文书）胡桃实心胶囊；次按钮（进入工作区）描边胶囊

色板不是豆包蓝，禁止系统蓝 CTA。挂上材料并解析任务卡后可「生成文书」，再进入三栏工作区。
...
3. Run。首页是居中对话栏。点空态胶囊「海天乙方电费+停车费报告」或底栏「示例」，再点实心「生成文书」进入三栏稿纸。核对缺口（电价P、全场灯数N、2018–2021 表）。
