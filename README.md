...
- 助手确认是纸面短文，不套紧白气泡。落稿后在奶油纸色上展开文书正文 + 紧凑「下载 .docx」，聊天里不贴真表。
- 生成后给 2–3 条「相关问题」编号链，点选即发出短追问。不编造台账数字。
- `home.generate` 挂在最后一条可生成确认上（文字链，不是中段大号双胶囊）。
...
3. Run。打开即见左话题 + 右聊天。点「示例」后：用户气泡上方是文件卡片，助手是纸面短确认 +「生成文书」文字链，中间没有材料 chip 条。生成后纸面展开正文、紧凑「下载 .docx」和「相关问题」。
