...
第一屏是 `HomeChatView`：大衬线「律书」+ 一句说明，中间消息流，底部多行输入 + 衬线「发送」。次级动作：「选材料文件夹」·「导入文件」·「载入示例案件」。空态有示例 chips（含「海天乙方电费+停车费报告」）。

布局/交互像对话；色板不是豆包蓝。输入框与检索同一细胡桃边。挂上材料并解析任务卡后，回执提供「生成文书」，再进入三栏工作区。
...
3. Run。首页是对话。点 chip「海天乙方电费+停车费报告」或「载入示例案件」，再点「生成文书」进入三栏稿纸。核对缺口（电价P、全场灯数N、2018–2021 表）。
